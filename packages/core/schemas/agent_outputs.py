...
class RequirementAnalysisOutput(BaseModel):
    ...
    user_stories: list[dict[str, Any]] = Field(
        default_factory=list, description="List of user stories with acceptance criteria"
    )
    ...
    complexity_estimate: str = Field(
        default="medium", description="Complexity estimate: low, medium, high"
    )
    suggested_technologies: list[str] = Field(
        default_factory=list, description="Suggested technology stack"
    )
...
class SystemDesignOutput(BaseModel):
    ...
    api_specs: list[APISpec] = Field(default_factory=list, description="API specifications")
    database_schema: list[DatabaseTable] = Field(
        default_factory=list, description="Database schema design"
    )
    external_dependencies: list[str] = Field(
        default_factory=list, description="External services/libraries needed"
    )
    security_considerations: list[str] = Field(
        default_factory=list, description="Security measures to implement"
    )
    scalability_plan: str = Field(default="", description="How the system scales")
    deployment_strategy: str = Field(default="", description="Recommended deployment approach")
...
class CodingOutput(BaseModel):
    ...
    dependencies: dict[str, Any] = Field(
        default_factory=dict, description="Package dependencies (npm, pip, etc.)"
    )
    ...
    setup_instructions: list[str] = Field(
        default_factory=list, description="Steps to set up the project"
    )
    testing_instructions: list[str] = Field(default_factory=list, description="How to run tests")
...
class CodeReviewOutput(BaseModel):
    ...
    positive_points: list[str] = Field(default_factory=list, description="What's done well")
    quality_score: int = Field(..., ge=0, le=100, description="Overall quality score 0-100")
    security_score: int = Field(default=0, ge=0, le=100, description="Security assessment 0-100")
    performance_score: int = Field(
        default=0, ge=0, le=100, description="Performance assessment 0-100"
    )
    maintainability_score: int = Field(
        default=0, ge=0, le=100, description="Maintainability assessment 0-100"
    )
...
class TestCase(BaseModel):
    ...
    prerequisites: list[str] = Field(default_factory=list, description="Setup needed before test")
...
class TestingOutput(BaseModel):
    ...
    test_cases: list[TestCase] = Field(default_factory=list, description="Defined test cases")
    test_files: list[CodeFile] = Field(
        default_factory=list, description="Generated test code files"
    )
    coverage_target: int = Field(
        default=80, ge=0, le=100, description="Target code coverage percentage"
    )
    execution_results: list[TestResult] = Field(
        default_factory=list, description="Results from test execution"
    )
    coverage_report: dict[str, Any] = Field(default_factory=dict, description="Coverage statistics")
    issues_found: list[dict[str, Any]] = Field(
        default_factory=list, description="Bugs/issues discovered"
    )
    recommendation: str = Field(..., description="PASS, PASS_WITH_WARNINGS, or FAIL")
