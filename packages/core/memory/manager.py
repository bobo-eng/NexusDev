...
from core.memory.retrieval.vector import SimpleVectorStore, VectorStore
...
class MemoryManager:
    ...
    async def add_code_dependency(
        ...
        """
        from core.memory.retrieval.graph import GraphEdge, GraphNode
    ...
    async def compress_old_memories(
        ...
        if session_id:
            entries = await self.working.query(
                MemoryQuery(
                    session_id=session_id,
                    limit=1000,
                )
