...
from core.domain.stage import Stage, StageType, StageStatus
...
class TestOpenClawAdapter:
    ...
    def test_extract_artifacts(self):
        ...
        artifacts = adapter.extract_artifacts_from_skill(
            skill_output, session_id, stage_id
        )
...
class TestAdapterCompatibility:
    ...
    def test_both_adapters_handle_same_artifact(self):
        ...
        claw_output = {
            "artifacts": [
                {"name": "test.py", "type": "source_code", "content": "pass"}
            ]
        }
        ...
        claw_artifacts = openclaw.extract_artifacts_from_skill(
            claw_output, session_id, stage_id
        )
        work_artifacts = openwork.extract_artifacts_from_plugin(
            work_output, session_id, stage_id
        )
